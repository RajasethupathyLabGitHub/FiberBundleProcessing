function [obj_out,center, Cn, PNR] = ini_addPoints(obj, K, save_avi, use_parallel, min_corr, min_pnr, seed_method,coords_in)
%% Edited by Andrew Toader, The Rockefeller University (based on initComponents_residual_parallel
%% input:
...
fprintf('\n---------------PICK NEURONS FROM MANUAL SELECTION----------------\n');
...
for mpatch=1:(nr_patch*nc_patch)
    ...
    ind_in_patch = [coords_in(:,1) > tmp_patch(3) & coords_in(:,1) < tmp_patch(4),...
                      coords_in(:,2) > tmp_patch(1) & coords_in(:,2) < tmp_patch(2)];
    coords_in_patch = find(sum(ind_in_patch,2)==2);
    ...
    if(~isempty(coords_in_patch))
        ...
        coords_relative = coords_in - [tmp_patch(3), tmp_patch(1)] + [1, 1];
        [tmp_results, tmp_center, tmp_Cn, tmp_PNR, ~] = ROI_addpoints(Ypatch, K, tmp_options, [], tmp_save_avi,coords_relative(coords_in_patch,:));
...
% in patch_pos, coordinates are ordered [y, x] (like you would index an
% image matrix in matlab (row, column)
%% collect results
for mpatch=1:(nr_patch*nc_patch)
    % get the indices corresponding to the selected patch
